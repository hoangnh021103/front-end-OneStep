...
export default {
  ...
  async mounted() {
    ...
    // Auto-refresh orders every 5s to reflect admin status changes quickly
    this._ordersInterval = setInterval(async () => {
      console.log('🔄 Auto-refresh: Cập nhật trạng thái đơn hàng từ server...')
      await this.loadOrders()
      console.log('✅ Đã cập nhật trạng thái mới từ server')
    }, 5000) // Giảm xuống 5 giây để cập nhật nhanh hơn
  },
  unmounted() {
