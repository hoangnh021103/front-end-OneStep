          ...
          <div class="form-grid">
            ...
            <div class="form-group">
              ...
              <select v-model="newInvoice.status" :class="{ 'error': errors.status }">
                <option value="pending" :disabled="!isStatusAllowed('pending')">Chờ xác nhận</option>
                <option value="confirmed" :disabled="!isStatusAllowed('confirmed')">Đã xác nhận</option>
                <option value="shipping" :disabled="!isStatusAllowed('shipping')">Chờ giao</option>
                <option value="delivering" :disabled="!isStatusAllowed('delivering')">Đang giao</option>
                <option value="done" :disabled="!isStatusAllowed('done')">Hoàn thành</option>
                <option value="cancel" :disabled="!isStatusAllowed('cancel')">Đã hủy</option>
              </select>
...
export default {
  ...
  methods: {
    ...
    // Quy tắc: chỉ được giữ nguyên hoặc tiến 1 bước; có thể hủy nếu chưa hoàn thành
    isStatusAllowed(nextStatus) {
      const ordered = ['pending', 'confirmed', 'shipping', 'delivering', 'done'];
      const current = this.newInvoice?.status || 'pending';
      if (nextStatus === 'cancel') return current !== 'done';
      const currentIndex = ordered.indexOf(current);
      const nextIndex = ordered.indexOf(nextStatus);
      ...
      try {
        ...
        this.invoices = allInvoices.map((inv) => {
          return {
            ...
            // Khởi tạo số liệu sản phẩm, sẽ được điền sau khi load chi tiết
            soLuongSanPham: inv.soLuongSanPham ?? null,
            soLoaiSanPham: inv.soLoaiSanPham ?? null
          };
        });
        ...
        // Sau khi có danh sách, tải số liệu sản phẩm cho từng đơn
        await this.fetchProductCountsForInvoices();
    ...
    // Tải số lượng sản phẩm và số loại sản phẩm cho tất cả hóa đơn (song song)
    async fetchProductCountsForInvoices() {
      try {
        const tasks = this.invoices.map(inv => this.loadInvoiceProductCounts(inv));
        await Promise.all(tasks);
        this.$nextTick(() => {
          this.$forceUpdate();
        });
      } catch (e) {
        // Không chặn UI nếu lỗi; chỉ log
        console.warn('⚠️ Không thể tải số liệu sản phẩm cho một số hóa đơn:', e?.message || e);
      }
    },

    // Tải số liệu sản phẩm cho 1 hóa đơn (có cache)
    async loadInvoiceProductCounts(invoice) {
      if (!invoice || invoice.soLuongSanPham != null || invoice.soLoaiSanPham != null) {
        return;
      }
      try {
        const res = await axios.get(`http://localhost:8080/chi-tiet-don-hang/don-hang/${invoice.id}`, {
          timeout: 8000,
          headers: { 'Content-Type': 'application/json' }
        });
        const items = Array.isArray(res.data?.data) ? res.data.data : (Array.isArray(res.data) ? res.data : []);
        // Trường name fallback phổ biến từ API chi tiết: soLuong, soLuongMua, quantity
        const totalQty = items.reduce((sum, it) => sum + (it.soLuong ?? it.soLuongMua ?? it.quantity ?? 0), 0);
        // Đếm số loại theo ID sản phẩm, fallback theo độ dài danh sách
        const typeSet = new Set();
        items.forEach(it => {
          const productId = it.chiTietSanPham?.sanPham?.id || it.chiTietSanPham?.sanPhamId || it.sanPhamId || it.productId || it.id;
          typeSet.add(productId);
        });
        const typeCount = items.length === 0 ? 0 : typeSet.size;
        // Gán trực tiếp lên object để reactive
        invoice.soLuongSanPham = totalQty;
        invoice.soLoaiSanPham = typeCount;
      } catch (err) {
        console.warn('⚠️ Lỗi khi tải chi tiết đơn hàng để tính số liệu:', invoice?.id, err?.message || err);
        // Giữ nguyên null để UI có thể hiển thị 0 hoặc trạng thái khác
        invoice.soLuongSanPham = invoice.soLuongSanPham ?? 0;
        invoice.soLoaiSanPham = invoice.soLoaiSanPham ?? 0;
      }
    },
    ...
    async handleSubmit() {
      ...
      try {
        ...
        // Chặn chuyển trạng thái nhảy bước hoặc lùi bước
        const ordered = ['pending', 'confirmed', 'shipping', 'delivering', 'done'];
        const fromStatus = this.mapTrangThaiToStatus(this.newInvoice.originalTrangThai || this.newInvoice.trangThai);
        const toStatus = this.newInvoice.status;
        const fromIdx = ordered.indexOf(fromStatus);
        const toIdx = ordered.indexOf(toStatus);
        const isCancel = toStatus === 'cancel';
        const isValidTransition = isCancel ? fromStatus !== 'done' : (toIdx === fromIdx || toIdx === fromIdx + 1);
        if (!isValidTransition) {
          toast.error('Không thể chuyển trạng thái nhảy bước. Thứ tự hợp lệ: Chờ xác nhận → Đã xác nhận → Chờ giao → Đang giao → Hoàn thành. (Có thể Hủy trước khi Hoàn thành)');
    ...
    async viewInvoiceDetail(invoice) {
      ...
      try {
        ...
        if (response.data && response.data.success) {
          ...
          // Cập nhật số liệu sản phẩm vào danh sách hóa đơn
          const totalQty = this.getTotalQuantity();
          const typeSet = new Set();
          this.selectedInvoiceProducts.forEach(p => {
            const productId = p.chiTietSanPham?.sanPham?.id || p.chiTietSanPham?.sanPhamId || p.sanPhamId || p.id;
            typeSet.add(productId);
          });
          const typeCount = this.selectedInvoiceProducts.length === 0 ? 0 : typeSet.size;
          const indexInList = this.invoices.findIndex(i => i.id === invoice.id);
          if (indexInList !== -1) {
            this.$set ? this.$set(this.invoices[indexInList], 'soLuongSanPham', totalQty) : (this.invoices[indexInList].soLuongSanPham = totalQty);
            this.$set ? this.$set(this.invoices[indexInList], 'soLoaiSanPham', typeCount) : (this.invoices[indexInList].soLoaiSanPham = typeCount);
          }
        } else {
